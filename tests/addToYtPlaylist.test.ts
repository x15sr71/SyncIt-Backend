...
const getMockFunction = <T extends (...args: any[]) => any>(fn: T): jest.MockedFunction<T> =>
...
describe('addToYoutubePlaylist', () => {
  ...
  it('adds all new video IDs with no errors', async () => {
    ...
    const added = await addToYoutubePlaylist('user1', ['v1', 'v2'], 'playlist1');
  ...
  it('skips already existing videos', async () => {
    ...
    expect(
      (
        mockedAxios.post.mock.calls[0][1] as {
  ...
  it('retries on 401, refreshes token, then succeeds', async () => {
    mockedGetToken.mockResolvedValueOnce('badtoken').mockResolvedValueOnce('goodtoken');

    // Fake error object, must have .response.status = 401
    const err401 = { response: { status: 401 }, message: 'Unauthorized' };
    mockedAxios.get.mockRejectedValueOnce(err401).mockResolvedValueOnce({
      data: { items: [], nextPageToken: undefined },
    });
  ...
  it('throws error after exceeding maximum retries', async () => {
    ...
    await expect(addToYoutubePlaylist('user1', ['v1'], 'playlist1')).rejects.toThrow(
      /exceeded max retries/i,
    );
