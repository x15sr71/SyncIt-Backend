...
const removeTracksFromLiked = async (
  ...
  for (let i = 0; i < trackIds.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Removing batch ${Math.floor(i / BATCH_SIZE) + 1} of ${Math.ceil(trackIds.length / BATCH_SIZE)} (${batch.length} tracks)...`,
    );
    ...
    while (retryCount <= MAX_RETRIES) {
      try {
        ...
        retryCount++;
        console.log(
          `Retrying batch ${Math.floor(i / BATCH_SIZE) + 1}, attempt ${retryCount + 1}...`,
        );
      }
    }
  }
