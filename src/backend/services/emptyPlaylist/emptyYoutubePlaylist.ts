...
const handleYouTubeError = async (error, retryCount, userId) => {
  ...
  if (status === 401 && retryCount < MAX_RETRIES) {
    ...
    console.error(`YouTube API error (${status}):`, data?.error?.message || error.message);
  }
    ...
    while (attempt <= 3) {
      try {
        ...
        if (status === 409 && attempt < 3) {
          console.log(
            `409 Conflict deleting ${itemId}. Retrying (${attempt + 1}/3) after 500ms...`,
          );
...
export const clearYouTubePlaylist = async (userId: string, playlistId: string) => {
  let accessToken = await get_YoutubeAccessToken(userId);

  console.log(`Fetching items in playlist: ${playlistId}`);
  const playlistItemIds = await fetchPlaylistItemIds(playlistId, accessToken, [], null, userId);
